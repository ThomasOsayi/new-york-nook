...
function CateringNav() {
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 80, maxWidth: 1440, margin: "0 auto", padding: "0 clamp(20px,4vw,60px)" }}>
        <Link href="/" style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 14, textDecoration: "none" }}>
          <div style={{ width: 36, height: 36, border: "1px solid rgba(183,143,82,0.5)", display: "flex", alignItems: "center", justifyContent: "center", transform: "rotate(45deg)" }}>
            <span style={{ transform: "rotate(-45deg)", fontFamily: "var(--font-display)", fontSize: 16, color: "#C9A050", fontWeight: 700 }}>N</span>
          </div>
          <div>
            <span style={{ fontFamily: "var(--font-display)", fontSize: 18, fontWeight: 700, color: "#fff", letterSpacing: 3, display: "block", lineHeight: 1 }}>NEW YORK NOOK</span>
            <span style={{ fontFamily: "var(--font-body)", fontSize: 8, fontWeight: 400, letterSpacing: 4, color: "rgba(183,143,82,0.6)", textTransform: "uppercase" as const }}>Fine Russian Cuisine</span>
          </div>
        </Link>
        ...
        <button
          ...
          style={{ display: "none", background: "none", border: "none", cursor: "pointer", padding: 8, flexDirection: "column" as const, gap: 5 }}
        >
          <span style={{ width: 22, height: 1.5, background: "#C9A050", display: "block", transition: "all 0.3s", transform: open ? "rotate(45deg) translate(2px,2px)" : "" }} />
          <span style={{ width: 22, height: 1.5, background: "#C9A050", display: "block", transition: "all 0.3s", opacity: open ? 0 : 1 }} />
          <span style={{ width: open ? 22 : 15, height: 1.5, background: "#C9A050", display: "block", transition: "all 0.3s", transform: open ? "rotate(-45deg) translate(4px,-4px)" : "" }} />
        </button>
      </div>

      {open && (
        <div style={{ position: "fixed", inset: 0, top: 80, background: "rgba(8,6,3,0.98)", backdropFilter: "blur(40px)", display: "flex", flexDirection: "column" as const, alignItems: "center", justifyContent: "center", gap: 28, zIndex: 300 }}>
          {NAV_LINKS.map((item, i) => (
            <Link
              ...
              style={{
                textDecoration: "none", fontFamily: "var(--font-display)", fontSize: 32, fontWeight: 400,
                color: item.label === "Catering" ? "#C9A050" : "#fff",
                letterSpacing: 3, animation: `fadeUp 0.5s ease ${i * 0.06}s forwards`, opacity: 0,
              }}
            >
              {item.label}
            </Link>
